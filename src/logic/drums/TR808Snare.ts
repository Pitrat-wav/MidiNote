...
export class TR808Snare {
    ...
    trigger(time: number, pitch: number, snappy: number, velocity: number = 0.8) {
        ...
        const gainLow = new Tone.Gain((1 - toneBalance) * velocity);
        const gainHigh = new Tone.Gain(toneBalance * velocity);
        ...
        masterTonalGain.gain.setValueAtTime(1 * velocity, time);
        ...
        // High-pass filter (>1800Hz) with drift
        const noiseFilter = new Tone.Filter(1800 * filterDrift, "highpass");
