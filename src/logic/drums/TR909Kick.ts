...
export class TR909Kick {
    ...
    trigger(time: number, pitch: number, decay: number, velocity: number = 0.8) {
        ...
        const bodyOsc = new Tone.Oscillator(tune + drift, "triangle");
        ...
        const startFreq = (tune + drift) * 4.7;
        ...
        const noiseFilter = new Tone.Filter(1000 * (1 + (Math.random() * 0.04 - 0.02)), "highpass"); // HPF with drift
        ...
        // Ultra short envelope (10-20ms) for the click, also scaled by velocity
