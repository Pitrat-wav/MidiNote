...
export function SequencerLoop() {
    ...
    useEffect(() => {
        ...
        const loop = new Tone.Loop((time) => {
            ...
            // Helper for drum trigger with probability
            const triggerWithProb = (drum: 'kick' | 'snare' | 'hihat' | 'hihatOpen' | 'clap' | 'cowbell') => {
                if (patterns[drum][step % patterns[drum].length]) {
                    if (Math.random() < currentDrums[drum].probability) {
                        drumMachine.triggerDrum(drum, time, getVel())
                    }
                }
            }

            triggerWithProb('kick')
            triggerWithProb('snare')
            triggerWithProb('hihat')
            triggerWithProb('hihatOpen')
            triggerWithProb('clap')
            triggerWithProb('cowbell')
