...
export class TR808Clap {
    ...
    trigger(time: number, pitch: number, decay: number) {
        ...
        // Analog drift for filter
        const filterFreq = (1000 + pitch * 1000) * (1 + (Math.random() * 0.04 - 0.02));
        const bpf = new Tone.Filter(filterFreq, "bandpass");
        ...
        // Triple attack "snaps"
        const snapCount = 3;
        const snapInterval = 0.01 + (Math.random() * 0.002 - 0.001); // Randomize snap interval slightly
