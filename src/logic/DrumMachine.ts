...
export class DrumMachine {
    ...
    constructor() {
        // Research: Master bus glue effect - Compressor + WaveShaper
        this.comp = new Tone.Compressor(-24, 4)
        this.shaper = new Tone.WaveShaper(this.makeDistortionCurve(15))
        this.shaper.oversample = '4x' // Research: reduce aliasing
        ...
        // Routing: All drum outputs -> Compressor -> WaveShaper -> Master Output -> Destination
    ...
    private makeDistortionCurve(amount: number) {
        // Research Formula: (3 + k) * x * 20 * deg / (Math.PI + k * Math.abs(x))
        const k = amount
    ...
    triggerDrum(drum: 'kick' | 'snare' | 'hihat' | 'hihatOpen' | 'clap', time: number, velocity: number = 0.8) {
        ...
        if (this.currentKit === '808') {
            ...
            switch (drum) {
                ...
                case 'hihatOpen':
                    // In TR909 open hats are also available
