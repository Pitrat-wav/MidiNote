...
export function SequencerLoop() {
    ...
    useEffect(() => {
        ...
        const loop = new Tone.Loop((time) => {
            ...
            // Helper for probability-based drum trigger
            const tryTrigger = (drum: 'kick' | 'snare' | 'hihat' | 'hihatOpen' | 'clap' | 'cowbell') => {
                const pattern = patterns[drum]
                if (pattern[step % pattern.length]) {
                    const prob = currentDrums[drum].probability
                    if (Math.random() < prob) {
