...
export class TR808Snare {
    ...
    trigger(time: number, pitch: number, snappy: number) {
        // Research: Tone control balances low and high modes
        const toneBalance = pitch;

        // Research: 808 Membrane modes fixed at 238Hz and 476Hz
        const drift = (Math.random() * 2 - 1) * 2; // micro-randomization
        const freqLow = 238 + drift;
        const freqHigh = 476 + drift;
        ...
        // Research: Snappy range 0.25s - 0.4s
