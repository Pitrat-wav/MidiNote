...
export function SequencerLoop() {
    ...
    useEffect(() => {
        ...
        const loop = new Tone.Loop((time) => {
            ...
            // Trigger drums with slight velocity randomization (0.7 - 1.0) to simulate analog variance
            const vel = () => 0.7 + Math.random() * 0.3
            if (patterns.kick[step % patterns.kick.length]) drumMachine.triggerDrum('kick', time, vel())
            if (patterns.snare[step % patterns.snare.length]) drumMachine.triggerDrum('snare', time, vel())
            if (patterns.hihat[step % patterns.hihat.length]) drumMachine.triggerDrum('hihat', time, vel())
            if (patterns.hihatOpen[step % patterns.hihatOpen.length]) drumMachine.triggerDrum('hihatOpen', time, vel())
            if (patterns.clap[step % patterns.clap.length]) drumMachine.triggerDrum('clap', time, vel())
