...
export class DrumMachine {
    ...
    constructor() {
        // Master Bus: Compression + Tanh Saturation (Glue Effect)
        ...
        // Routing: All instruments -> Comp -> Shaper -> Master Gain -> Destination
    ...
    private makeDistortionCurve(amount: number) {
        ...
        for (let i = 0; i < n_samples; ++i) {
            ...
            // Soft clipping tanh approximation from research
    ...
    triggerDrum(drum: 'kick' | 'snare' | 'hihat' | 'hihatOpen' | 'clap', time: number, velocity: number = 0.8) {
        ...
        // Add subtle variations to pitch (+/- 1%) and decay (+/- 2%)
        const pitchDrift = p.pitch * (1 + (Math.random() * 0.02 - 0.01))
        const decayDrift = p.decay * (1 + (Math.random() * 0.04 - 0.02))
        ...
        if (this.currentKit === '808') {
            switch (drum) {
                case 'kick': kit808.kick.trigger(time, pitchDrift, decayDrift); break
                case 'snare': kit808.snare.trigger(time, pitchDrift, decayDrift); break
                case 'hihat': kit808.hihat.trigger(time, false, pitchDrift, decayDrift); break
                case 'hihatOpen': kit808.hihat.trigger(time, true, pitchDrift, decayDrift); break
                case 'clap': kit808.clap.trigger(time, pitchDrift, decayDrift); break
            }
        } else {
            switch (drum) {
                case 'kick': kit909.kick.trigger(time, pitchDrift, decayDrift); break
                case 'snare': kit909.snare.trigger(time, pitchDrift, decayDrift); break
                case 'hihat': kit909.hihat.trigger(time, false, pitchDrift, decayDrift); break
                case 'hihatOpen': kit909.hihat.trigger(time, true, pitchDrift, decayDrift); break
                case 'clap': kit909.clap.trigger(time, pitchDrift, decayDrift); break
            }
        }
    }
}
