...
export class TR808HiHat {
    ...
    trigger(time: number, isOpen: boolean, pitch: number, decay: number) {
        ...
        const oscillators = this.frequencies.map(freq => {
            // Analog drift: +/- 2Hz per report
            const drift = (Math.random() - 0.5) * 4;
            // Map pitch (0-1) to a multiplier (e.g. 0.8x to 1.2x) to maintain tunability while staying faithful to hardware ratios
        ...
        // Parallel Bandpass Filters per report: 3440Hz and 7100Hz, Q=1.5
        ...
        // Final HPF per report: 7000Hz cleanup
        ...
        // Precise decay times: 40-60ms (Closed), 300-500ms (Open) per report
        const closedDecay = 0.04 + decay * 0.02;
        const openDecay = 0.3 + decay * 0.2;
        const decayTime = isOpen ? openDecay : closedDecay;
        ...
        // Disposal - using a single onstop on the first oscillator to trigger cleanup of all related nodes
