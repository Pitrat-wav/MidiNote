...
export class TR909Kick {
    ...
    trigger(time: number, pitch: number, decay: number) {
        ...
        const noiseFilterFreq = 1000 * (1 + (Math.random() * 0.04 - 0.02));
        ...
        // Micro-randomization: Click decay variance (+/- 2%)
        ...
        // Dispose of body nodes when bodyOsc stops
        bodyOsc.onstop = () => {
            bodyOsc.dispose();
            bodyGain.dispose();
        };
        // Dispose of noise nodes when noiseSrc ends
        noiseSrc.onended = () => {
            noiseSrc.dispose();
            noiseFilter.dispose();
            noiseGain.dispose();
