...
export class TR909Kick {
    ...
    trigger(time: number, pitch: number, decay: number, velocity: number = 0.8) {
        ...
        // Research: Add a smoothing low-pass filter to the triangle body (~1000Hz)
        ...
        // Research: Tune controls the frequency sweep duration (0.05s to 0.15s)
        const startFreq = tune * 4.7 + drift;
        const endFreq = tune + drift;
        const sweepTime = 0.05 + pitch * 0.10; // 50ms - 150ms
        ...
        // Ultra short envelope (10-20ms) for the click, scaled by velocity
