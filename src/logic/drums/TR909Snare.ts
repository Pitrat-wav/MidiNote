...
export class TR909Snare {
    ...
    trigger(time: number, pitch: number, snappy: number) {
        ...
        // 2x Pitch Sweep over 30ms (adjusted to 30ms based on 909 Snare research)
        ...
        // Micro-randomization: Tonal Decay Time (+/- 2%)
        let tonalDecay = 0.2;
        tonalDecay *= (1 + (Math.random() * 0.04 - 0.02));
        tonalGain.gain.exponentialRampToValueAtTime(0.001, time + tonalDecay);

        // Snappy Layer
        const noiseSrc = new Tone.BufferSource(this.noiseBuffer);

        // Micro-randomization: Snappy Filter Cutoff (+/- 2%)
        ...
        // Micro-randomization: Snappy Decay Time (+/- 2%)
